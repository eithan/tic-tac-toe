# Hybrid FastAPI application that works with or without AlphaZero
from fastapi import FastAPI, HTTPException, Request
from fastapi.middleware.cors import CORSMiddleware
import uvicorn
import os

# Try to import the full GameService with AlphaZero support
try:
    from tic_tac_toe.game.game_service import GameService
    ALPHAZERO_AVAILABLE = True
    print("✅ AlphaZero support enabled")
except ImportError as e:
    print(f"⚠️  AlphaZero not available: {e}")
    print("🔄 Falling back to basic game logic")
    from game_logic import GameService
    ALPHAZERO_AVAILABLE = False

# Store the import error for debugging
ALPHAZERO_ERROR = str(e) if not ALPHAZERO_AVAILABLE else None
...
app = FastAPI(
    ...
    description=f"Tic-Tac-Toe game API ({ENVIRONMENT} mode) - AlphaZero: {'Available' if ALPHAZERO_AVAILABLE else 'Not Available'}",
    version="1.0.0"
)
...
if IS_PRODUCTION or os.getenv("ENABLE_SECURITY", "false").lower() == "true":
    ...
    print(f"🔒 Security features enabled ({ENVIRONMENT} mode)")
else:
...
def get_allowed_origins():
    ...
    if IS_PRODUCTION:
        ...
        origins.extend([
            ...
            "https://dialogic-unponderous-melody.ngrok-free.app",
        ])
...
async def health_check():
    ...
    return {
        ...
        "alphazero_available": ALPHAZERO_AVAILABLE,
        "alphazero_error": ALPHAZERO_ERROR,
...
async def debug_info():
    """Debug endpoint to check AlphaZero availability."""
    import sys
    import os
    ...
    packages_to_check = ["tensorflow", "open_spiel", "neuralnet", "tic_tac_toe"]
    for package in packages_to_check:
        try:
            __import__(package)
            debug_info["installed_packages"].append(f"{package}: ✅")
        except ImportError as e:
            debug_info["installed_packages"].append(f"{package}: ❌ ({e})")
...
if __name__ == "__main__":
    ...
    print(f"🤖 AlphaZero: {'Available' if ALPHAZERO_AVAILABLE else 'Not Available'}")
